...
bool fAddressIndex = false; // ariel
...
bool addToGlobalForkTips(const CBlockIndex* pindex)
{
    ...
    if (erased == 0)
    {
        LogPrintf("%s():%d - adding first fork tip in global map: h(%d) [%s]\n",
...
int getMostRecentGlobalForkTips(std::vector<uint256>& output)
{
    ...
    std::vector<map_pair> vTemp(std::begin(mGlobalForkTips), std::end(mGlobalForkTips));

    sort(std::begin(vTemp), std::end(vTemp), [](const map_pair& a, const map_pair& b) { return a.second < b.second; });
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    if (pindex->nHeight > m_params.GetConsensus().nSmartActivationBlock){
        globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // ariel
        globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // ariel
    }
...
//    std::cout<<"pos1 dgpMaxBlockSize " << dgpMaxBlockSize <<std::endl;
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // ariel
...
////////////////////////////////////////////////////////////////// // ariel
    ...
    if((checkBlock.GetHash() != block.GetHash()) && !fJustCheck)
    {
        ...
        if(checkBlock.hashStateRoot != block.hashStateRoot){
            ...
            LogPrintf("checkBlock.ToString() (%s)\n", checkBlock.ToString());
...
bool CChainState::ConnectTip(BlockValidationState& state, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions& disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // ariel
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // ariel
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // ariel
            globalState->setRootUTXO(oldHashUTXORoot); // ariel
 ...
 */
bool CChainState::ActivateBestChainStep(BlockValidationState& state, CBlockIndex* pindexMostWork, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, ConnectTrace& connectTrace, bool &postponeRelay)
{
    AssertLockHeld(cs_main);
    if (m_mempool) AssertLockHeld(m_mempool->cs);
    postponeRelay = false;
    ...
    do {
...

        {
            ...
            do {
                ...
                if (!ActivateBestChainStep(state, pindexMostWork, pblock && pblock->GetHash() == pindexMostWork->GetBlockHash() ? pblock : nullBlockPtr, fInvalidFound, connectTrace, postponeRelayTmp)) {
